...
| 版本 | 要点 |
| --- | --- |
| v1.x（旧·答题版） | 答题测评版「处世 Radar」，9 维 81 题。**完整保留在 `legacy-v1` 分支**（及 `v1.0.0` 标签），不随新版本删除 |
| v2.0 | 四合一重构：雷达/人脉/记录/修炼，原生 JS + ECharts |
| v2.1–v2.6 | 农历节日提醒、日记本美术、安装引导、待办管理、维度解读、年度报告、备份迁移、批量修 bug |
| v2.7 | 品牌体系（Logo/Slogan/欢迎引导/关于页）、提醒紧急度颜色修复 |

> 分支说明：`main` = 当前经营版（v2.x）；`legacy-v1` = 旧答题版（处世 Radar v1，完整源码）。
